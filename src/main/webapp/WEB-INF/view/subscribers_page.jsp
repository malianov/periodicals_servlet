    ...
    <table class="table table-hover table-bordered border-primary border border-2 text-muted">
        ...
        <tr>
            ...
            <td class="text-center">
                <c:choose>
                    <c:when test="${all_subscribers_list.getUserStatus() eq 'BLOCKED'}">
                        <form method="get" action="/app/app/to_block_unblock_user?search_input=${search_input}&current_page=${i}" style="margin-bottom: 0px;">
        ...
        <ul class="pagination justify-content-center ">
            <c:if test="${current_page != 1}">
                <li class="page-item"><a
                        href="${pageContext.request.contextPath}/app/to_subscribers_page?search_input=${search_input}&current_page=${current_page - 1}"
                        class="page-link border-primary"><fmt:message key="pagination.previous"/></a></li>
            </c:if>
            <c:if test="${current_page == 1}">
                <li class="page-item"><a
                        href="${pageContext.request.contextPath}/app/to_subscribers_page?search_input=${search_input}&current_page=${current_page}"
            ...
            <c:forEach begin="1" end="${nu_of_pages}" var="i">
                <c:choose>
                    <c:when test="${current_page eq i}">
                        <li class="page-item"><a
                                href="${pageContext.request.contextPath}/app/to_subscribers_page?search_input=${search_input}&current_page=${i}"
